  ...
  class Logic
    class DamageHandler < ChangeHandlerBase
      ...
      def handle_boss_bar_removal(hp, target, launcher, skill, drain_factor, source, &messages)
        ...
        handle_drain_effects(hp, target, launcher, skill, drain_factor) if source == :drain
        breaks.times { handle_bar_break_event(hp, target, skill) }

        return display_bar_loss_message(target, breaks) unless path.knocked_out
      ...
      # Trigger the post-death events for a single broken bar. It is not a real KO, so the launcher is
      # suppressed (its on-KO rewards must not proc); only the boss-side effects run.
      # @param hp [Integer] The amount of HP (damage) dealt.
      # @param target [PFM::PokemonBattler] The target Pokémon receiving the damage.
      # @param skill [Battle::Move, nil] The move used, if applicable.
      def handle_bar_break_event(hp, target, skill)
        handle_post_damage_death_events(hp, target, nil, skill)
        target.ko_count += 1
      end
    end
  end
