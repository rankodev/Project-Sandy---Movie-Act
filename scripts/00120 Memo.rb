    ...
    class Summary_Memo < SpriteStack
      ...
      def data=(pokemon)
        if (self.visible = !pokemon.nil?)
          ...
          @type1_sprite.x = pokemon.type2 == 0 ? 275 : 241
      ...
      def init_memo
        ...
        with_surface(114, 19, 95) do
          # --- Static part ---
          add_line(0, 'Species') # Nom
          no_egg add_line(1, 'Level') # NoPokedex
          ...
          no_egg add_line(3, texts[10]) # Pt exp
          no_egg add_line(4, texts[12]) # Next lvl
          no_egg add_line(5, text_get(23, 7)) # Objet
          # --- Data part ---
          ...
          no_egg add_line(3, :exp_text, 2, type: SymText, color: 1, dx: 1)
          no_egg add_line(4, :exp_remaining_text, 2, type: SymText, color: 1, dx: 1)
          no_egg add_line(5, :item_name, 2, type: SymText, color: 1, dx: 1)
        end
        # no_egg add_text(114, 19 + 16 * 3, 92, 16, :trainer_name, 2, type: SymText, color: 1)
        @type1_sprite = no_egg push(241, 19 + 34, nil, type: Type1Sprite)
        @type2_sprite = no_egg push(275, 19 + 34, nil, type: Type2Sprite)
      end
