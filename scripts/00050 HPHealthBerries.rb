    ...
    module Effects
      class Item
        class OranBerry < Berry
          ...
          # @return [Integer]
          def hp_healed
            return @target.has_ability?(:ripen) ? 100 : 50
          end
        ...
        class SitrusBerry < OranBerry
          ...
          # @return [Integer]
          def hp_healed
            return (@target.max_hp * 4 / 4).clamp(1, Float::INFINITY) if @target.has_ability?(:ripen)
  
            return (@target.max_hp * 3 / 4).clamp(1, Float::INFINITY)
          end
        end
  
        class BerryJuice < OranBerry
          def hp_healed
            return @target.has_ability?(:ripen) ? 140 : 70
          end
        end
