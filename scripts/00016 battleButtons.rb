module BattleUI
    ...
    class PlayerChoice < GenericChoice
      remove_const :BUTTON_COORDINATE
      
      include UI
      include PlayerChoiceAbstraction
      # Coordinate of each buttons
      BUTTON_SELECT_COORDINATE = [[17, 203], [89, 203], [159, 203], [231, 203]]
      BUTTON_COORDINATE = [[17, 205], [89, 205], [159, 205], [231, 205]]
      ...
      def create_buttons
        # @type [Array<Button>]
        add_background('battle/choice_background')
        #@what_will_do = add_text(100, 197, 0, 16, text_get(20, 85), color: 10) Asks what pokemon will do
        @buttons = 4.times.map do |i|
          arr = get_button_position(@index == i, i)
          add_sprite(*arr, NO_INITIAL_IMAGE, i, type: Button)
        end
      end
      ...
      def update_key_index
        ...
        if Input.trigger?(:LEFT)
          @index = @index == 0 ? 3 :(@index - 1).clamp(0, 3)
        elsif Input.trigger?(:RIGHT)
          @index = @index == 3 ? 0 :(@index + 1).clamp(0, 3)
        end
        update_buttons if @index != old_index
      end
      
      def update_buttons
        @buttons.each do |button|
          is_currently_selected = (@index == button.index)
          button.update_selected_button(is_currently_selected)
          arr = get_button_position(is_currently_selected, button.index)
          button.set_position(*arr)
        end
      end
      
      def get_button_position(selected, button_index)
        x = selected ? BUTTON_SELECT_COORDINATE[button_index][0] : BUTTON_COORDINATE[button_index][0]
        x += 0 if @index == 0 && button_index == 1
        x -= 0 if @index == 3 && button_index == 2
        y = selected ? BUTTON_SELECT_COORDINATE[button_index][1] : BUTTON_COORDINATE[button_index][1]
        return x, y
      end
      ...
      def update
        old_index = @index
        super
        update_buttons if old_index != @index
      end
      ...
      class Button < SpriteSheet
        ...
        def update_selected_button(bool)
          set_bitmap(bool ? 'battle/actions_selected_' : 'battle/actions_', :interface)
        end
        ...
        # @return [String]
        def image_filename
          return self.index == 0 ? 'battle/actions_selected_' : 'battle/actions_'
        end
      end
      ...
      class SpecialButton < UI::SpriteStack
        ...
        public
        def create_sprites #INFORMATION and LAST ITEM USED TEXT
          # TODO: separate in methods
          add_background(@type == :info ? 'battle/button_y' : 'battle/button_x')
          @text = add_text(1, 8, 0, 16, nil.to_s, color: 10)
      ...
      class SubChoice < UI::SpriteStack
        ...
        def create_special_buttons
          @last_item_button = add_sprite(25, -135, NO_INITIAL_IMAGE, :last_item, type: SpecialButton)
          @info_button = add_sprite(25, -160, NO_INITIAL_IMAGE, :info, type: SpecialButton)
        end
